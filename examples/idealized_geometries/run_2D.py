...
def solve_system():
    ...
    for tag, subdomain in subdomain_list.items():
        ...
        xdmf_sub[tag] = xdmf
        fname_bp_sub[tag] = f"results/2D/checkpoint_sub_{tag}.bp"

        # Write membrane potential to file for all cellular subdomains (i.e. all subdomain but ECS)
        if tag > 0:
            xdmf = dolfinx.io.XDMFFile(comm, f"results/2D/results_mem_{tag}.xdmf", "w")
            xdmf.write_mesh(subdomain['mesh_mem'])
            adios4dolfinx.write_mesh(f"results/2D/checkpoint_mem_{tag}.bp", subdomain['mesh_mem'])
            xdmf_mem[tag] = xdmf
            fname_bp_mem[tag] = f"results/2D/checkpoint_mem_{tag}.bp"
